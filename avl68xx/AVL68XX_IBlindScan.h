...
/// 
#ifndef AVL68XX_IBlindScan_h_h
	#define AVL68XX_IBlindScan_h_h
...
extern "C" {
	...
	/// Stores the blind scan parameters which are passed to the ::AVL68XX_DVBSx_BlindScanPara function.
	struct AVL68XX_DVBSx_BlindScanPara
	{
	...
	struct AVL68XX_DVBSx_BlindScanInfo
	{
	...
	/// Performs a blind scan operation. Call the function ::AVL68XX_DVBSx_IBlindScan_Scan to check the status of the blind scan operation.
	/// 
	...
	/// @param uiChipNo The Demo chip id
	/// 
	/// @return ::AVL_ErrorCode, 
	/// Return ::AVL_EC_OK if the scan command is successfully sent to the Availink device.
	/// Return ::AVL_EC_I2C_FAIL if there is an I2C communication problem.
	/// Return ::AVL_EC_RUNNING if the scan command could not be sent because the Availink device is still processing a previous command.
	/// Return ::AVL_EC_GENERAL_FAIL if the device is not in the blind scan functional mode or if the parameter \a pBSPara->m_uiStartFreq_100kHz is larger than the parameter \a pBSPara->m_uiStopFreq_100kHz
	...
	/// 
	/// @return ::AVL_ErrorCode, 
	/// Return ::AVL_EC_OK if the blind scan status has been retrieved.
	/// Return ::AVL_EC_I2C_FAIL if there is an I2C communication problem.
	...
	/// 
	/// @return ::AVL_ErrorCode, 
	/// Return ::AVL_EC_OK if the Availink device has been commanded to cancel the blind scan operation.
	...
	/// 
	/// @return ::AVL_ErrorCode, 
	/// Return ::AVL_EC_OK if the blind scan results have been retrieved.
	/// Return ::AVL_DVBSx_EC_I2CFail if there is an I2C communication problem.
