...
///$Date: 2012-3-8 21:47 $
///
#ifndef AVL68XX_IBlindScanAPI_h_h
	#define AVL68XX_IBlindScanAPI_h_h
...
extern "C" {
	...
	enum AVL68XX_DVBSx_BlindScanAPI_Status
	{	
		AVL68XX_DVBSx_BS_Status_Init = 0,							///< = 0 Indicates that the blind scan process is initializing the parameters.
		AVL68XX_DVBSx_BS_Status_Start = 1,							///< = 1 Indicates that the blind scan process is starting to scan.
		AVL68XX_DVBSx_BS_Status_Wait = 2,							///< = 2 Indicates that the blind scan process is waiting for the completion of scanning.
		AVL68XX_DVBSx_BS_Status_Adjust = 3,							///< = 3 Indicates that the blind scan process is reading the channel info which have scanned out.
		AVL68XX_DVBSx_BS_Status_User_Process = 4,					///< = 4 Indicates that the blind scan process is in custom code. Customer can add the callback function in this stage such as adding TP information to TP list or lock the TP for parsing PSI.
		AVL68XX_DVBSx_BS_Status_Cancel = 5,							///< = 5 Indicates that the blind scan process is cancelled or the blind scan have completed.
		AVL68XX_DVBSx_BS_Status_Exit = 6							///< = 6 Indicates that the blind scan process have ended.
	};

	/// 
	/// Defines the blind scan mode.
	typedef enum AVL68XX_DVBSx_BlindScanAPI_Mode
	{
		AVL68XX_DVBSx_BS_Fast_Mode = 0,								///< = 0 Indicates that the blind scan frequency step is automatic settings.
		AVL68XX_DVBSx_BS_Slow_Mode = 1								///< = 1 Indicates that the blind scan frequency step can be setting by user. The default value is 10MHz.
	}AVL68XX_DVBSx_BlindScanAPI_Mode;

	/// 
	/// Stores the blind scan configuration parameters.
	struct AVL68XX_DVBSx_BlindScanAPI_Setting
	{
		...
		AVL68XX_DVBSx_BlindScanAPI_Mode  BS_Mode;				///< The blind scan mode. \sa ::AVL_DVBSx_IBlindScanAPI_Mode.
		...
		AVL_ChannelInfo  channels[128];				///< Stores the channel information that all scan out results. 
		AVL_ChannelInfo  channels_Temp[16];			///< Stores the channel information temporarily that scan out results by the blind scan procedure.

		AVL_BlindScanPara  bsPara;					///< Stores the blind scan parameters each blind scan procedure.
		AVL_BSInfo  bsInfo;					///< Stores the blind scan status information each blind scan procedure.
		AVL_BlindSanSpectrumPolarity  m_eSpectrumMode;		///< Defines the device spectrum polarity setting.  \sa ::AVL_DVBSx_SpectrumPolarity.
	};
